...
class parser
{
	...
	function parse( $text )
	{
					...
					// tag zamykajacy
					{
						if( $value[7] == $this -> config['close_string'][0] && $value[9] == $this -> config['close_string'][1] )
						{
							if( array_key_exists($value[8],$this -> tags) )
							{
								...
								{
									// tagi w blednej kolejnosci
									return false;
								}
							}
						} else
						{
							$this -> error('blad parseru! nieznane partie znakow obejmujace tagi bbcode!',__LINE__);
						}
					}
				}
			}
		...
		private function special_chars( $text )
		{
			...
			// błędne kodowanie m.in. z phpmyadmin
